...
export const getMyBourseLots = functions.region('europe-west1').https.onCall(async (data, context) => {
  const uid = context.auth?.uid
  if (!uid) throw new functions.https.HttpsError('unauthenticated', 'Login required')

  const limit = Math.min(((data as { limit?: number })?.limit ?? 20), 100)

  const snap = await db.collection('bourse_opportunities')
    .where('createdBy', '==', uid)
