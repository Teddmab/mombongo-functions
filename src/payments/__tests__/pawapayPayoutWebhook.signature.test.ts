...
function fakeReq(overrides: Record<string, unknown> = {}) {
  return {
    ...
    body: { payoutId: 'pay1', status: 'COMPLETED' },
    rawBody: Buffer.from(JSON.stringify({ payoutId: 'pay1', status: 'COMPLETED' })),
    ...overrides,
  }
}

describe('pawapayPayoutWebhook — RFC 9421 signature enforcement', () => {
  beforeEach(() => verifyMock.mockReset())

  it('rejects the request when verification fails, never reaching business logic', async () => {
    verifyMock.mockResolvedValue(false)
    const res = fakeRes()
    await (pawapayPayoutWebhook as unknown as Handler)(fakeReq(), res)
  ...
  it('passes the raw body Buffer, not JSON.stringify(req.body), to the verifier', async () => {
    verifyMock.mockResolvedValue(false)
    const req = fakeReq()
    await (pawapayPayoutWebhook as unknown as Handler)(req, fakeRes())
  ...
  it('proceeds to business logic once verification succeeds', async () => {
    ...
    await expect((pawapayPayoutWebhook as unknown as Handler)(fakeReq(), fakeRes())).rejects.toThrow('BUSINESS_LOGIC_REACHED')
  })
})
